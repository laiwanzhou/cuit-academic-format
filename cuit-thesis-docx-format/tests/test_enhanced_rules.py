...
def test_remove_toc_field_placeholder_blank_after_english_keywords():
    ...
    assert any(mod._paragraph_has_toc_field(p) for p in doc.paragraphs)
...
def test_toc_field_detected_from_instr_text():
    mod = load_module()
    doc = mod.Document()
    p = doc.add_paragraph("")
    r = p.add_run()
    ...
    instr.text = ' TOC \\o "1-3" \\h \\z \\u '
    r._r.append(instr)
    assert mod.detect_toc_field_paragraph(p) is True
    start, end = mod.detect_toc_field_range(doc)
    assert start == 0 and end == 0


def test_front_matter_page_numbering_lower_roman():
    mod = load_module()
    doc = mod.Document()
    ...
    doc.add_section(WD_SECTION_START.NEW_PAGE)
    doc.add_paragraph("1 引 言")
    mod.apply_page_number_formats(doc)
    fmt_front, _ = mod._section_page_number_format(doc.sections[1])
    assert fmt_front == "lowerRoman"


def test_body_page_numbering_decimal_restart_one():
    mod = load_module()
    doc = mod.Document()
    doc.add_paragraph("封面")
    doc.add_section(WD_SECTION_START.NEW_PAGE)
    ...
    doc.add_paragraph("1 引 言")
    mod.apply_page_number_formats(doc)
    fmt_body, start_body = mod._section_page_number_format(doc.sections[2])
    assert fmt_body == "decimal"
    assert start_body == "1"


def test_toc_and_body_same_section_are_split_or_reported():
    mod = load_module()
    doc = mod.Document()
    doc.add_paragraph("封面")
    doc.add_section(WD_SECTION_START.NEW_PAGE)
    ...
    r._r.append(instr)
    doc.add_paragraph("1 引 言")
    changed = mod.ensure_body_starts_new_page_number_section(doc)
    if changed:
        assert len(doc.sections) >= 3
    else:
        issues = mod.collect_section_issues(doc)
        assert any(i.rule_key in {"toc_body_same_section", "front_page_number"} for i in issues)


def test_toc_entry_page_numbers_not_converted_to_roman():
    mod = load_module()
    doc = mod.Document()
    doc.add_paragraph("目 录")
    p1 = doc.add_paragraph("1 引 言 .... 1")
    p2 = doc.add_paragraph("1.1 课题背景 .... 1")
    mod.apply_page_number_formats(doc)
    assert p1.text.endswith("1")
    assert p2.text.endswith("1")


def test_no_duplicate_page_fields_in_footer():
    mod = load_module()
    doc = mod.Document()
    doc.add_paragraph("封面")
    doc.add_section(WD_SECTION_START.NEW_PAGE)
    ...
    mod.apply_page_number_formats(doc)
    field_instr = []
    for node in doc.sections[1].footer._element.iter():
        if node.tag == mod.qn("w:instrText") and node.text:
            field_instr.append(node.text)
    page_count = sum(1 for t in field_instr if "PAGE" in t.upper())
    assert page_count == 1


def test_page_number_fix_does_not_touch_reference_summary():
    mod = load_module()
    doc = mod.Document()
    doc.add_paragraph("参考文献")
    doc.add_paragraph("[1] 张三. 题名[J]. 刊名, 2020, 1(2):3-5.")
    ...
    entries, _ = mod._reference_entries_from_regions(doc.paragraphs, texts, ["references", "references"])
    summary = mod._reference_272_check_summary(entries)
    assert summary["entries"][0]["numbering_source"] in {"text", "word_auto_numbering", "word_auto_numbering_assumed_continuous", "inferred"}
    assert summary["entries"][0]["visual_index"] == 1
...
def test_output_files_include_same_run_timestamp():
    ...
    mod.probe_com_renderer = lambda _r: "office"
    mod.apply_supported_rules = lambda *_args, **_kwargs: None
    mod.try_com_save = lambda *_args, **_kwargs: "office"
    mod.cleanup_visible_blank_paragraphs_after_layout = lambda _d: ([], False)
    mod.attach_after_formats = lambda *_args, **_kwargs: None
    mod.attach_screenshots = lambda **_kwargs: ("skipped", None)
    mod.write_html_report = lambda *_args, **_kwargs: None
    mod.compute_section_boundary_findings = lambda *_args, **_kwargs: ([], [])
    mod.build_run_timestamp = lambda: "20260513_154230"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        out = base / "out"
        report = mod.run(docx, out, "auto", "never")
        assert report["run_timestamp"] == "20260513_154230"
        for key in ["annotated_docx", "fixed_docx", "json_report", "html_report"]:
            assert "20260513_154230" in report[key]


def test_report_paths_use_timestamped_names():
    mod = load_module()
    path = Path("D:/tmp/sample_format_fixed.docx")
    out = mod.append_timestamp_to_name(path, "20260513_154230")
    assert out.name.endswith("_20260513_154230.docx")


def test_repeated_runs_do_not_overwrite_previous_outputs():
    mod = load_module()
    path = Path("D:/tmp/sample_format_report.json")
    p1 = mod.append_timestamp_to_name(path, "20260513_154230")
    p2 = mod.append_timestamp_to_name(path, "20260513_154231")
    assert p1 != p2
