...
未经用户明确要求，不得调整已经完成且通过验证的功能代码。每次任务只允许修改本次需求直接相关的最小代码范围；禁止顺手重构、顺手优化、顺手改风格、顺手改其它模块。若发现其它模块存在问题，只在结果中说明，不主动修改。若本次需求必须触碰公共函数，必须说明原因，并补充回归测试，证明页眉、封面、摘要空段、图片保护、正文分页等既有功能未被破坏。
